...
impl Function {
    fn call_verbose(
        ...
        match self {
            ...
            Function::Variable { value, constant, name } => {
                if args.len() == 0 {
                    value.clone()
                } else if !*constant {
                    let pass = value.clone();
                    scope.borrow_mut().set_function(name, Function::Variable {
                        value: args[0].clone(),
                        constant: false,
    ...
    pub fn call(&self, args: Vec<Data>, yield_fn: Option<Function>, scope: ScopeRef) -> Data {
        self.call_verbose(args, yield_fn, scope, false, true, None)
    }

    pub fn call_scope(&self, args: Vec<Data>, yield_fn: Option<Function>, scope: ScopeRef) -> Data {
...
impl Debug for Function {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Variable { value, constant, name: _ } =>
                f
