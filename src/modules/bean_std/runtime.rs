...
use crate::{
	arg_check, as_mut_type, as_type,
	data::{Data, DataType},
	error::{Error, ErrorSource},
	modules::{loader, CustomModule, ModuleBuilder},
	scope::{
		block_scope::{BlockScope, IfState},
		function::Function,
		Scope, ScopeRef,
	},
};
...
fn fn_fn(
	...
	let body_fn =
		body_fn.unwrap_or_else(|| panic!("To define a function, add a body block."));
...
fn fn_call(
	...
	let function = scope.borrow().get_function(name);
	if let None = function {
		return Err(Error::new(
			&format!("Unknown value or function {}", name),
			ErrorSource::Builtin(String::from("call")),
		));
	}
...
fn fn_export(
	...
	let module =
		as_mut_type!(binding => CustomModule, "Tried to export from a non-module scope.");
	...
	if let Function::Variable { .. } = target {
		return Err(Error::new(
			"Cannot export a variable.",
			ErrorSource::Builtin(String::from("export")),
		));
	}
...
fn fn_use(args: Vec<Data>, _y: Option<Function>, scope: ScopeRef) -> Result<Data, Error> {
	...
	let (path_str, target) = {
		...
		(
			iter.next().expect("Tried to import from blank path."),
			iter.next(),
		)
	};
	...
	if target == Some("*") {
		...
		name_scope.borrow_mut().set_function(
			name_str,
			module.borrow().get_function(t).unwrap_or_else(|| {
				panic!(
					"Tried to import non-existent function {} from module {}.",
					t, path_str
				)
			}),
		);
...
fn fn_args(_a: Vec<Data>, _y: Option<Function>, scope: ScopeRef) -> Result<Data, Error> {
	Ok(Data::Scope(Rc::new(RefCell::new(List::new(
		Vec::clone(
			&scope
				.borrow()
				.get_call_scope()
...
fn fn_rand(args: Vec<Data>, _y: Option<Function>, _s: ScopeRef) -> Result<Data, Error> {
	match args.len() {
		...
		2.. => {
			...
			Ok(Data::Number(
				(rand::random::<f64>() * (max - min)).floor() + min,
			))
		}
	}
}
...
fn fn_num(args: Vec<Data>, _y: Option<Function>, _s: ScopeRef) -> Result<Data, Error> {
	match args.get(0).unwrap_or(&Data::None) {
		...
		Data::String(s) => Ok(s.parse().map(|v| Data::Number(v)).unwrap_or(Data::None)),
...
fn fn_type(args: Vec<Data>, _y: Option<Function>, _s: ScopeRef) -> Result<Data, Error> {
	Ok(Data::String(
		args.get(0).unwrap_or(&Data::None).get_type().to_string(),
	))
}
...
fn fn_map(args: Vec<Data>, _y: Option<Function>, scope: ScopeRef) -> Result<Data, Error> {
	Ok(Data::Scope(Rc::new(RefCell::new(Map::new(
		args,
		Some(scope),
	)))))
}
...
fn fn_eq(args: Vec<Data>, _y: Option<Function>, _s: ScopeRef) -> Result<Data, Error> {
	Ok(Data::Boolean(
		args.get(0).unwrap_or(&Data::None) == args.get(1).unwrap_or(&Data::None),
	))
}
...
fn fn_else_if(
	...
	let if_state = scope.borrow().get_if_state().ok_or(Error::new(
		"Cannot use else if conditionals in this scope.",
		ErrorSource::Builtin(String::from("else_if")),
	))?;

	match if_state {
		IfState::Started => {
			if *v {
				scope.borrow_mut().set_if_state(IfState::Captured);
				body_fn
					.unwrap_or_else(|| panic!("To define a variable, add a body block."))
					.call_direct(Vec::new(), None, Rc::clone(&scope))?;
			} else {
				scope.borrow_mut().set_if_state(IfState::Started);
			}
			Ok(Data::None)
		}
		IfState::Captured => Ok(Data::None),
		IfState::Finished => Err(Error::new(
			"Tried to call else_if before if.",
			ErrorSource::Builtin(String::from("else_if")),
		)),
	}
}
...
fn fn_else(
	...
	match if_state {
		IfState::Started => {
			scope.borrow_mut().set_if_state(IfState::Finished);
			body_fn
				.unwrap_or_else(|| panic!("To define a variable, add a body block."))
				.call_direct(Vec::new(), None, Rc::clone(&scope))?;
			Ok(Data::None)
		...
		IfState::Finished => Err(Error::new(
			"Tried to call else before if.",
			ErrorSource::Builtin(String::from("else")),
		)),
	}
}
...
fn fn_repeat(
	...
	let body_fn = body_fn.ok_or(Error::new(
		"Expected body fn.",
		ErrorSource::Builtin(String::from("repeat")),
	))?;
...
fn fn_while(
	...
	let body_fn = body_fn.ok_or(Error::new(
		"Expected body fn.",
		ErrorSource::Builtin(String::from("while")),
	))?;
...
fn fn_match(
	...
	body_fn.expect("Expected body for fn match").call_direct(
		Vec::new(),
		None,
		Rc::clone(&match_scope) as ScopeRef,
	)
}
