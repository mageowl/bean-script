...
pub fn evaluate_verbose(
    ...
    match node {
        Node::FnCall { name, parameters, yield_fn } => {
            let function = scope
                .get_function(&name)
            ...
            for n in parameters {
                args.push(evaluate(n, Rc::clone(access_scope_ref.as_ref().unwrap_or(&scope_ref))));
            }
            ...
            let return_value = function.call_from(
                ...
                if let Some(body) = yield_fn {
                    Some(Function::Custom {
                        ...
                        scope_ref: Rc::clone(access_scope_ref.as_ref().unwrap_or(&scope_ref)),
                    })
                } else {
                    None
                },
        ...
        Node::Scope { body } => {
            ...
            return if return_scope { Data::Scope(scope_ref) } else { return_value };
        }
        ...
        Node::FnAccess { target, call } => {
            ...
            if let Data::Scope(target_scope) = target {
                drop(scope);
                evaluate_verbose(call, Rc::clone(&target_scope), false, Some(Rc::clone(&scope_ref)))
            } else {
